...
class Nigel
{
    public:
        ...
        /* Result will be {} (empty) if we failed */
        pplx::task<std::vector<WalletTypes::WalletBlockInfo>> getWalletSyncData(
            const std::vector<Crypto::Hash> blockHashCheckpoints,
            uint64_t startHeight,
            uint64_t startTimestamp) const;

        /* Returns a bool on success or not */
        pplx::task<bool> getTransactionsStatus(
        ...
        pplx::task<std::vector<CryptoNote::RandomOuts>> getRandomOutsByAmounts(
            const std::vector<uint64_t> amounts,
            const uint64_t requestedOuts) const;

        /* {success, timeout} - In the case of timeout, timeout = true,
           success = false. Success indicates other failure if timeout = false,
           e.g. daemon didn't like our tx */
        pplx::task<std::tuple<bool, bool>> sendTransaction(
    ...
    private:
        ...
        pplx::task<void> getDaemonInfo();

        pplx::task<void> getFeeInfo();

        //////////////////////////////
        /* Private member variables */
        //////////////////////////////

        /* Stores our http client config, notable containing the timeout */
        web::http::client::http_client_config m_config;

        /* Stores our http client */
        std::shared_ptr<web::http::client::http_client> m_httpClient = nullptr;
