    ...
    const Crypto::Hash transactionHash)
{
    ...
    while (true)
    {
        const auto indexes = m_daemon->getGlobalIndexesForRange(startHeight, endHeight).get();
...
void WalletSynchronizer::downloadBlocks()
{
    ...
    while (!m_shouldStop)
    {
        ...
        const auto blocks = m_daemon->getWalletSyncData(
            std::move(blockCheckpoints), m_startHeight, m_startTimestamp
        ).get();

        /* If we get no blocks, we are fully synced.
           (Or timed out/failed to get blocks) Sleep a bit so we don't spam
           the daemon. */
        if (blocks.empty())
        {
            Utilities::sleepUnlessStopping(std::chrono::seconds(5), m_shouldStop);
